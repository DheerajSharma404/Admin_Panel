    ...
    return (
        <div className="flex flex-col h-screen p-6 w-full">
            <Modal isOpen={showEditModal} onClose={() => setShowEditModal(false)}>
                <EditProfile handleSubmit={handleSubmit} handleInputChange={handleInputChange} userDetails={userDetails} setShowEditModal={setShowEditModal} />
            </Modal>
            <h1 className="text-3xl font-bold mb-6">User Profile</h1>
            <div className="flex space-x-6">
                <div className="max-w-sm overflow-hidden bg-white rounded-lg shadow-lg hover:shadow-blue-400">
                    <div className="relative">
                        <img className="w-full h-48 object-cover" src="/assets/profile.jpg" alt="Profile Image" />
                    </div>
                    <div className="px-6 py-4">
                        <div className="text-xl font-semibold text-gray-800">{userDetails.name}</div>
                ...
                {showDetails && (
                    <div className="flex-1 overflow-hidden bg-white rounded-lg p-6">
                        ...
                        <div className="space-y-2">
                            ...
                            <p><span className="font-semibold">Position:</span> {userDetails.position}</p>
                            <p><span className="font-semibold">Location:</span> {userDetails.location}</p>
                            <p><span className="font-semibold">Bio:</span> {userDetails.bio}</p>
                        </div>
                        <button
