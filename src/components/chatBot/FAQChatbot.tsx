...
const FAQChatbot: React.FC<FAQChatbotProps> = ({ setIsChatbotOpen }) => {
  ...
  const findAnswer = (query: string): { text: string; isUser: false } => {
    ...
    if (matchedItem) {
      return { text: matchedItem.answer, isUser: false };
    } else {
      return {
        text: "I'm sorry, I couldn't find an answer to your question. Please try rephrasing or ask something else.",
        isUser: false,
      };
    }
  };
